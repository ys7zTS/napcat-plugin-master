import { isMaster, Master } from './index'
...
export const addMaster = createPlugin(/^#添加主人(\d{5,10})$/, async (event, ctx) => {
  ...
  if (isMaster(userId)) return await sendMsg(ctx, event, `(｡･ω･｡) ${userId} 已经是主人了哦`)
...
export const delMaster = createPlugin(/^#删除主人(\d{5,10})$/, async (event, ctx) => {
  ...
  if (!match) return false
  const userId = match[1]
  if (!isMaster(userId)) return await sendMsg(ctx, event, `(｡･ω･｡) ${userId} 不是主人哦`)
